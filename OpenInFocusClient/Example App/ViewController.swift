...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        swiftButton.setTitle("OpenInFocus with Swift", for: UIControlState())
        ...
        objCButton.setTitle("OpenInFocus with Obj-C", for: UIControlState())
    ...
    @objc func buttonActionSwift(_ sender: UIButton!) {
        let controller = OpenInFocusControllerSwift()
        if controller.isFocusInstalled() {
            controller.openInFocus(url)
        }
    }

    @objc func buttonActionObjC(_ sender: UIButton!) {
        let controller = OpenInFocusControllerObjC()
        if controller.isFocusInstalled() {
            controller.open(inFocus: url)
        }
    }
}
